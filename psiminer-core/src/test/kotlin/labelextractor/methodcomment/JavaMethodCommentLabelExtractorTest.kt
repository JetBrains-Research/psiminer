...
internal class JavaMethodCommentLabelExtractorTest : JavaPsiRequiredTest("JavaMethods") {
    ...
    fun `test method name extraction`(methodName: String, commentLabel: String) = ReadAction.run<Exception> {
        ...
        val methodCommentExtracted = methodCommentLabelExtractor.handleTree(psiRoot, handler)?.getStringRepresentation()
        Assert.assertEquals(commentLabel, methodCommentExtracted)
    }
    ...
    private fun provideParameters(): Stream<Arguments> {
        return Stream.of(
            ...
            Arguments.of("largeMethod", "this|is|java|doc|test|comment|another|comment")
        )
    }
}
