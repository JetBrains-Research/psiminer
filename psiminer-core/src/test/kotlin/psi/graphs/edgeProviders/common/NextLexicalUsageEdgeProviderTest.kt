...
internal class NextLexicalUsageEdgeProviderTest : JavaPsiRequiredTest("JavaFlowMethods") {
    ...
    fun `test next lexical usage extraction from Java methods`(methodName: String) {
        val psiRoot = getMethod(methodName)
        val graphMiner = JavaGraphMiner()
        ReadAction.run<Exception> {
            val codeGraph = graphMiner.mine(psiRoot)
            val nextLexicalUsageEdges =
                codeGraph.edges.withType(EdgeType.NextLexicalUsage).flatMap { (_, edges) -> edges.forward() }
            val textRepresentation = nextLexicalUsageEdges.map { Pair(getText(it.from), getText(it.to)) }.toSet()
